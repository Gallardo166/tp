...
MALAddress is a desktop address book application adapted from AddressBook Level 3 (AB3). It targets hawker stall owners
and staff, helping them manage supplier and regular-customer contacts efficiently. The app is written in Java, uses a
CLI for interaction, and has a JavaFX GUI.
...
* **New Feature**: Supplier/customer tagging + remarks (`tag` command)
    * What it does: Allows users to label contacts as `supplier` or `customer` (and other tags like `regular`) and
      attach short remarks for quick reference.
    * Why it matters: Helps hawker staff separate suppliers vs customers and store operational notes (e.g., supply type,
      preferences).
    * PR(s): [#??](LINK)
